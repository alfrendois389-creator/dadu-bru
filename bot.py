...
def init_db():
    conn = get_db()
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT, saldo REAL DEFAULT 0, dana TEXT DEFAULT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
    c.execute('CREATE TABLE IF NOT EXISTS history (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, type TEXT, amount REAL, description TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
    c.execute('CREATE TABLE IF NOT EXISTS withdraw_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, amount REAL, dana TEXT, status TEXT DEFAULT "pending", created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
    c.execute('CREATE TABLE IF NOT EXISTS last_win (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, username TEXT, amount REAL, side TEXT, score TEXT, game INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
    c.execute('CREATE TABLE IF NOT EXISTS deposits (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, amount INTEGER, order_id TEXT, status TEXT DEFAULT "pending", created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
    conn.commit()
    conn.close()
...
def add_history(user_id, typ, amount, desc=""):
    conn = get_db()
    c = conn.cursor()
    c.execute("INSERT INTO history (user_id, type, amount, description) VALUES (?, ?, ?, ?)", (user_id, typ, amount, desc))
    conn.commit()
    conn.close()
...
def save_last_win(user_id, username, amount, side, score):
    ...
    c.execute("INSERT INTO last_win (user_id, username, amount, side, score, game) VALUES (?, ?, ?, ?, ?, ?)", (user_id, username, amount, side, score, game_num))
    conn.commit()
    conn.close()
...
def add_withdraw_request(user_id, amount, dana):
    conn = get_db()
    c = conn.cursor()
    c.execute("INSERT INTO withdraw_requests (user_id, amount, dana) VALUES (?, ?, ?)", (user_id, amount, dana))
    conn.commit()
    conn.close()
...
def get_pending_wd():
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT w.*, u.username FROM withdraw_requests w JOIN users u ON w.user_id = u.id WHERE w.status = "pending"')
    res = c.fetchall()
    conn.close()
    return res
...
async def balance(update, context):
    ...
    await update.message.reply_text(f"💰 SALDO: {data['saldo']:.2f} KOIN\n💵 1 KOIN = Rp {KOIN_RATE:,}\n📌 MIN BET {MIN_BET} • WITHDRAW {WD_MIN}")

async def setdana(update, context):
    user = update.effective_user
    if not context.args:
...
async def lastwin(update, context):
    all_last = get_all_last_win()
    if not all_last:
...
# ==================== AUTO ROLL ====================
async def auto_roll(update, context):
...
async def deposit(update, context):
    ...
    try:
        ...
        if amount < 1000:
            await update.message.reply_text("❌ MIN DEPOSIT 1000!")
            return
    except:
        await update.message.reply_text("❌ JUMLAH HARUS ANGKA!")
        return
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO deposits (user_id, amount, order_id, status) VALUES (?, ?, ?, 'pending')",
        (user.id, amount, order_id)
    )
    conn.commit()
    conn.close()
...
async def cekwd(update, context):
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("❌ HANYA ADMIN!")
        return
...
if __name__ == "__main__":
    ...
    c.execute('CREATE TABLE IF NOT EXISTS withdraw_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, amount REAL, dana TEXT, status TEXT DEFAULT "pending", created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
    c.execute('CREATE TABLE IF NOT EXISTS last_win (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, username TEXT, amount REAL, side TEXT, score TEXT, game INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
    c.execute('CREATE TABLE IF NOT EXISTS deposit_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, username TEXT, amount REAL, status TEXT DEFAULT "pending", created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
    conn.commit()
    conn.close()
...
async def kirim_notif(pesan):
    for admin_id in ADMIN_IDS:
        try:
            await bot_notif.send_message(admin_id, pesan)
        except:
            pass
...
async def start(update, context):
    ...
    msg += (
        ...
        "/autooff - Matikan auto roll\n"
        "/help - Bantuan\n\n"
        f"📌 HUBUNGI ADMIN: {ADMIN_USERNAME} untuk deposit/withdraw manual."
    )
    await update.message.reply_text(msg)
...
async def deposit(update, context):
    ...
    if is_admin and len(context.args) >= 2:
        ...
        try:
            await context.bot.send_message(target['id'], f"✅ DEPOSIT BERHASIL!\n👤 @{username}\n💰 +{amount:.2f} KOIN (Rp {amount*KOIN_RATE:,.0f})")
        except:
            pass
        return
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("INSERT INTO deposit_requests (user_id, username, amount, status) VALUES (?, ?, ?, 'pending')", (user.id, user.username, amount))
    dep_id = c.lastrowid
    conn.commit()
    conn.close()
...
async def kirim_bukti_callback(update, context):
...
async def handle_bukti(update, context):
    ...
    for admin_id in ADMIN_IDS:
        try:
            await context.bot.send_photo(admin_id, photo, caption=f"📥 BUKTI TRANSFER\n👤 USERNAME: @{user.username}\n🆔 ID: {user.id}\n💰 NOMINAL: {state['amount']} KOIN (Rp {state['amount']*KOIN_RATE:,.0f})\n📌 KONFIRMASI: /confirmdeposit @{user.username} {state['amount']}")
        except:
            pass
    await update.message.reply_text(f"✅ BUKTI TERKIRIM KE ADMIN!\n👤 @{user.username}\n💰 {state['amount']} KOIN\n⏳ TUNGGU KONFIRMASI ADMIN.")
    conn = get_db()
    c = conn.cursor()
    c.execute("UPDATE deposit_requests SET status = 'waiting_confirmation' WHERE id = ?", (dep_id,))
    conn.commit()
    conn.close()
    user_states[user.id] = {}

async def confirmdeposit(update, context):
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("❌ HANYA ADMIN!")
        return
    if len(context.args) < 2:
    ...
    if not user:
        ...
        conn.close()
        return
    c.execute("UPDATE deposit_requests SET status = 'done' WHERE user_id = ? AND status = 'waiting_confirmation'", (user['id'],))
    conn.commit()
    conn.close()
    update_saldo(user['id'], amount)
    ...
    await update.message.reply_text(f"✅ DEPOSIT @{username} +{amount:.2f} KOIN BERHASIL!")
    try:
        await context.bot.send_message(user['id'], f"✅ DEPOSIT BERHASIL!\n👤 @{username}\n💰 +{amount:.2f} KOIN (Rp {amount*KOIN_RATE:,.0f})")
    except:
        pass
